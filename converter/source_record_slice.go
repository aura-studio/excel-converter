...
type SourceRecordSlice struct {
	*SourceBase
	index int
}

func NewSourceRecordSlice(ctx *SourceContext, sheet Sheet, index int) *SourceRecordSlice {
	s := &SourceRecordSlice{
...
func (s *SourceRecordSlice) Type() SourceType {
	return SourceTypeRecordSlice
}

func (s *SourceRecordSlice) ParseSources() {
	headerField := s.sheet.GetHeaderField(0)
	...
	for index := 0; index < len(contents); {
		...
		case StructureTypeStructs, StructureTypeMap, StructureTypeRows, StructureTypeCols:
			if len(field.Args) > 0 {
				sheetName := field.Args[0]
				sheet := s.Excel().GetSheet(sheetName)
				sheet.ParseContent(field.Structure)
